...
module.exports = new EntitySchema({
	...
	relations: {
		user: {
			// type: "many-to-one"：多對一關係。代表當前這個實體（例如 UserSkill）可以有多條紀錄對應到同一個 User。
			type: "many-to-one",
			// target: "User"：關聯的目標實體名稱。必須是程式碼中定義的 Class 名稱（例如 @Entity("User")），而不是資料庫裡面的真實表名。
			target: "User",
			// joinColumn: { name: "user_id" }：在當前這張資料表中，用來儲存對應外鍵（Foreign Key）的欄位名稱叫作 user_id。
